...
CYPHER_GatherDB_Specific = """
    MATCH (c:Compound) where (c.identifier contains 'DB' AND (not c.vestige = True))
    OR (c.identifier contains 'DB' and (not exists (c.vestige)))
    RETURN c.identifier as db_id, c.chembl_id as chembl_id
"""

CYPHER_GatherPC_Drug = """
    MATCH (c:Compound)-[r:INCLUDES_PCiC]-(p:PharmacologicClass) RETURN c.identifier as c_id, p.identifier as p_id
"""
...
CYPHER_AddPC_Drug = """
    ...
    AND p.identifier = '{p_id}' CREATE (c)<-[r:INCLUDES_PCiC]-(p) SET r.unbiased = false,
    r.license = 'CC BY 4.0', r.source = 'DrugCentral'
"""
...
def drugcentral_process_psql(recs, session, pc_recs):
    ...
    for dc in drugbank_to_chembl:
        db_c.update({dc["db_id"]:dc["chembl_id"]})
        ...
        if not pr[3] is None and not pr[4] is None:
            if 'CHEMBL' in pr[6] and pr[6].strip() not in chembl_arr:
                ...
                if not rel_c in dc_processed:
                    dc_processed.append(str(pr[6]) + "-" + str(pr[5]))
            if 'DB' in pr[6] and pr[6].strip() in db_arr:
...
def update_pharm_drug(session, new_rels):
    ...
    for rel in extract_old_rels:
        old_rels.append(str(rel["c_id"]) + "-" + str(rel["p_id"]))
...
""".format(count_add=count_add, count_vestige=count_vestige)
